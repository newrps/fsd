...
| 3 | UART 명령 수신 (Jetson → STM32) | PD9 RX (USART3) | COBS+postcard+CRC, 115200 8N1 |
...
## Jetson↔NUCLEO 시리얼 path — ST-LINK VCP 권장

NUCLEO USART3 의 PD8 (TX) / PD9 (RX) 는 **default SB12, SB19 ON** 으로 ST-LINK MCU 에 연결되어 있고, ST-LINK 가 USB CDC (가상 시리얼) 로 노출한다. 따라서:

- 점퍼와이어 / morpho 헤더 핀 직결 **불필요**
- USB 케이블 1개 (NUCLEO ↔ Jetson) 로 양방향 통신
- Jetson 측 디바이스: `/dev/ttyACM0`
- 호스트 PC (개발) 측 디바이스: `COMx` (윈도우) — XShell / PuTTY 등 시리얼 모니터로 검증 가능

### 실측 (2026-05-10)

```
펌웨어: USART3 PD8/PD9 @ 115200 8N1
Jetson 명령: fsd-jetson serve --serial /dev/ttyACM0 --baud 115200

결과: seq 877..881, last_applied_seq 49 (5초 / 100ms cmd ≈ 50)
양방향 통신 검증 완료
```

### 점퍼와이어 직결을 시도하지 않는 이유

NUCLEO-H753ZI MB1364 의 default SB 매트릭스가 USART RX/TX 외부 헤더 라우팅을 끊어 두는 형태:
- Zio CN10 의 D0/D1 (PB6/PB7) → 별도 SB (SB8, SB68 등) 솔더링 필요
- morpho CN12 의 PB10/PB11 → 헤더 핀 카운팅 어렵고, 정확한 핀 식별/배선 검증이 인두/멀티미터 없이는 어려움

ST-LINK VCP 가 **유일한 default 동작 path** 라 운용/개발 모두 USB 사용 권장. 실차 진동에 USB 커넥터 헐거워질 위험은 케이블 타이/글루로 고정.

USB 분리/재연결 시 chip 자동 부팅. probe-rs run 종료 시 chip halt 가능 (SWD detach 동작) — 부팅이 필요하면 USB power-cycle.
...
- LED1 깜빡이지만 명령 무반응: UART 배선/baud 확인. 115200 8N1
