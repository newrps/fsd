...
- **재개 가이드 문서 작성** (`docs/resume-after-parts.md`): 부품 (수-암 점퍼와이어 / 빵판 / 멀티미터 / 저항) 도착 후 즉시 따라갈 수 있는 단계별 절차. 7개 단계 (서보/ESC 결선 → bench 테스트 → RC 수신기 → 엔코더 → 배터리 분배기 → 카메라 → ml 모델 학습 후 자율주행) 각각 결선/명령/예상 결과/트러블슈팅 포함. docs/README.md 에 링크 추가.
- **ml-py 파이프라인 동작 검증** (PC venv): smoke (synthetic→train→ONNX→replay, MAE_s 0.12 / latency 308µs) + compare_archs (PilotNet vs TinyPilotNet, tiny 우승: MAE_s 0.03 / latency 147µs / ONNX 4.4KB) + drive_sim --auto (5초 / 30fps / 151 frames) + pytest (24/24) 모두 통과. 부품 대기 동안 sw 측 진행 가능 path 확인.
- **EXTI 입력 task polling 미동작 원인 규명**: RC 수신기 / 엔코더 미결선 상태에서 `ExtiInput::new(PA0/PA1/PA2, Pull::Down/Up)` + `bind_interrupts!{ EXTI0/1/2 }` 활성 시 floating-like 입력의 환경 노이즈로 IRQ 폭주 → executor 가 task wake 만 끝없이 처리해서 polling 정지 (heartbeat tick 도 안 찍힘). 단계적 진단 (step5 5-task 정상 → step6 EXTI 추가 시 멈춤 → EXTI 만 다시 제거 시 회복) 으로 원인 좁힘. 펌웨어 측: EXTI/RC capture/encoder/battery task 비활성화 (commented 블록 + `#[allow(dead_code)]`), main 에는 결선 후 활성화 가이드 주석. RC 수신기 결선 + pull 안정화 후 활성화. [troubleshooting.md](troubleshooting.md) 항목 추가.
- **펌웨어 원본 9-task 구조 복원** (`firmware/src/main.rs`, main.rs.bak 기반): heartbeat / safe_indicator / uart_rx_task / pwm_task / telemetry_task 5개는 활성, RC capture × 2 / encoder / battery 4개는 결선 전까지 비활성. USART3 PD8/PD9 + 115200 baud (ST-LINK VCP path 유지). LD1 sanity check + main alive 디버그 루프는 의도적으로 보존 — 부팅 진단에 유용.
