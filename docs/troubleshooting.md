...
### NUCLEO ↔ Jetson — 점퍼와이어 직결이 안 통하면 ST-LINK VCP 사용
> **증상**: NUCLEO 의 USART TX/RX 핀에 점퍼와이어 직결 후 Jetson 의 `/dev/ttyTHS1` 에서 0 byte. raw read (`dd if=/dev/ttyTHS1 bs=1 count=200`) 로도 데이터 없음.

NUCLEO-H753ZI MB1364 의 **default SB 매트릭스가 USART 신호의 외부 헤더 라우팅을 끊어 둔 상태**다 (Zio D0/D1 = PB6/PB7 는 SB8/SB68 ON 필요, morpho 직결은 핀 카운팅이 어렵고 SB 변경이 필요한 경우 많음). 솔더 브릿지 변경 작업은 인두 + 멀티미터 + schematic 없이 사실상 불가능.

해결: **PD8/PD9 USART3 + USB-VCP path 를 default 로 사용**.
- 펌웨어: `BufferedUart::new(p.USART3, p.PD9, p.PD8, ...)`
- 보드 측 와이어링: 없음 (SB12, SB19 default ON 이 PD8/PD9 ↔ ST-LINK MCU 자동 연결)
- 호스트: USB 케이블 1개 → `/dev/ttyACM0` (Jetson) 또는 `COMx` (Windows)
...
probe-rs 가 SWD attach 한 채로 종료 시 chip 을 halt 상태로 두는 동작. **USB 분리/재연결 (= power cycle)** 하면 펌웨어 자동 부팅. 또는 백그라운드로 `cargo run --release &` 띄워두면 attach 유지.
