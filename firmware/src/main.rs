...
//! - USART3 TX: PD8  (default SB12/SB19 ON 으로 ST-LINK VCP 와 자동 연결)
//! - USART3 RX: PD9  (Jetson 측 디바이스: /dev/ttyACM0)
//!   참고: 점퍼와이어 직결 (Zio CN10 PB10/PB11 또는 morpho 직결) 은
//!   default SB 라우팅 + 핀 식별 어려움으로 시도 실패. ST-LINK USB-VCP 가
//!   사실상 default 통신 path (USB 케이블 1개만 필요). 자세한 내용:
//!   docs/firmware.md, docs/troubleshooting.md.
...
async fn main(spawner: Spawner) {
    ...
    // PD8 (TX) / PD9 (RX) → ST-LINK MCU → USB CDC. 호스트 측 /dev/ttyACM0 또는 COMx.
    let uart = BufferedUart::new(p.USART3, p.PD9, p.PD8, tx_buf, rx_buf, Irqs, uart_cfg)
        .expect("USART3 init");
    let (uart_tx, uart_rx) = uart.split();

    // RC 캡처 (PA0/PA1) + 휠 엔코더 (PA2) 는 RC 수신기/엔코더 결선 후 활성화.
    // 미결선 상태에서 ExtiInput::new + bind_interrupts!{ EXTI0/1/2 } 를 enable 하면,
    // 입력 핀이 floating-like 상태에서 IRQ 폭주를 일으켜 executor 의 task polling 이
    // 멈춘다 (확인됨 2026-05-10, docs/troubleshooting.md). 결선 후 아래 commented 블록
    // 활성화 + bind_interrupts! 에 EXTI0/1/2 + use embassy_stm32::interrupt::typelevel
    // 도 같이 복원.
    let _ = (p.PA0, p.EXTI0, p.PA1, p.EXTI1, p.PA2, p.EXTI2);
    ...
    // battery_task — 외부 4:1 분배기 결선 + ADC 자체적 starvation 검토 후 활성화.
    let _ = (p.ADC1, p.PC0);
...
// 아래 RC capture / encoder / battery task 들은 외부 결선 후 main 의 spawn 활성화
// (commented 블록 참고). 미결선 상태에서 ExtiInput 활성 시 IRQ 폭주로 polling 멈춤.
#[allow(dead_code)]
#[embassy_executor::task]
